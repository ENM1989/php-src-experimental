        ...
        case NODE_FOR_STATEMENT: {
            ...
            if (item && item->type == CONTEXT_TYPE_LIST) {
                for (int i = 0; i < item->data.list.count; i++) {
                    ...
                    ContextItem *current_item_head = item->data.list.items[i]->head;
                    while(current_item_head) {
                        if (current_item_head->type == CONTEXT_TYPE_STRING) {
                            context_set_string(loop_ctx, current_item_head->key, current_item_head->data.value);
                        }
                        current_item_head = current_item_head->next;
                    }
