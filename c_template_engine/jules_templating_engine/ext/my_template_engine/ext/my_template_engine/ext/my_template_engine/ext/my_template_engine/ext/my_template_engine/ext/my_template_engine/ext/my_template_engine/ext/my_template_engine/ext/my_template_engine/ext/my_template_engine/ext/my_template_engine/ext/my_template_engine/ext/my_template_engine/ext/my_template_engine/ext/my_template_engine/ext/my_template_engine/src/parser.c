...
static IfStatementNode *parse_if_statement_node(Parser *p);
static ForStatementNode *parse_for_statement_node(Parser *p);
static RootNode *parse_block(Parser *p, const char *end_literal1, const char *end_literal2);
...
RootNode *parse_template(Parser *p) {
    return parse_block(p, NULL, NULL);
}
...
static RootNode *parse_block(Parser *p, const char *end_literal1, const char *end_literal2) {
    ...
    while (p->current_token.type != TOKEN_EOF) {
        if (p->current_token.type == TOKEN_STMT_START) {
            if (end_literal1 && strcmp(p->peek_token.literal, end_literal1) == 0) {
                break;
            }
            if (end_literal2 && strcmp(p->peek_token.literal, end_literal2) == 0) {
                break;
            }
        }
...
static Node *parse_statement(Parser *p) {
    switch (p->current_token.type) {
        ...
        case TOKEN_STMT_START:
            if (p->peek_token.type == TOKEN_IDENTIFIER) {
                if (strcmp(p->peek_token.literal, "if") == 0) {
                    parser_next_token(p);
                    return (Node *)parse_if_statement_node(p);
                }
                if (strcmp(p->peek_token.literal, "for") == 0) {
                    parser_next_token(p);
                    return (Node *)parse_for_statement_node(p);
                }
            }
            return NULL;
        default:
...
static IfStatementNode *parse_if_statement_node(Parser *p) {
    parser_next_token(p); // consume 'if'

    IfStatementNode *node = (IfStatementNode *)malloc(sizeof(IfStatementNode));
    node->base.type = NODE_IF_STATEMENT;
    node->condition = strdup(p->current_token.literal);

    parser_next_token(p); // consume condition
    if (p->current_token.type != TOKEN_STMT_END) {
    ...
    node->consequence = parse_block(p, "endif", "else");

    if (p->current_token.type == TOKEN_STMT_START && strcmp(p->peek_token.literal, "else") == 0) {
        parser_next_token(p); // consume {%
        parser_next_token(p); // consume else
        if (p->current_token.type != TOKEN_STMT_END) {
            fprintf(stderr, "Expected %%} after else\n");
        }
        node->alternative = parse_block(p, "endif", NULL);
    } else {
        node->alternative = NULL;
    }

    if (p->current_token.type != TOKEN_STMT_START || strcmp(p->peek_token.literal, "endif") != 0) {
        fprintf(stderr, "Expected {%% endif %%}\n");
    }
    parser_next_token(p); // consume {%
    parser_next_token(p); // consume endif

    return node;
}

static ForStatementNode *parse_for_statement_node(Parser *p) {
    parser_next_token(p); // consume 'for'
    ...
    node->loop_var = strdup(p->current_token.literal);

    parser_next_token(p);
    if (p->current_token.type != TOKEN_IN) return NULL;

    parser_next_token(p);
    if (p->current_token.type != TOKEN_IDENTIFIER) return NULL;
    node->collection_name = strdup(p->current_token.literal);

    parser_next_token(p);
    if (p->current_token.type != TOKEN_STMT_END) return NULL;

    node->body = parse_block(p, "endfor", NULL);

    if (p->current_token.type != TOKEN_STMT_START || strcmp(p->peek_token.literal, "endfor") != 0) {
        // error
    }
    parser_next_token(p); // consume {%
    parser_next_token(p); // consume endfor
