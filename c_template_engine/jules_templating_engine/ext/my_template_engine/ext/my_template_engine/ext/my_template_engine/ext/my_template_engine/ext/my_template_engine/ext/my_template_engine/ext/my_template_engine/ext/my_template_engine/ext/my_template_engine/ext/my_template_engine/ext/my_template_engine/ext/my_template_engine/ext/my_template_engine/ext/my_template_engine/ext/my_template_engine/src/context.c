...
void context_set_list(Context *ctx, const char *key, char **items, int count) {
    ...
    item->data.list.items = (char **)malloc(sizeof(char*) * count);
    for (int i = 0; i < count; i++) {
        item->data.list.items[i] = strdup(items[i]);
    }
...
Context *context_clone(const Context *ctx) {
    ...
    while (current) {
        ...
        if (current->type == CONTEXT_TYPE_STRING) {
            ...
            new_item->data.list.items = (char **)malloc(sizeof(char*) * new_item->data.list.count);
            for (int i = 0; i < new_item->data.list.count; i++) {
                new_item->data.list.items[i] = strdup(current->data.list.items[i]);
            }
        }
        new_item->next = NULL;
