        ...
        case NODE_FOR_STATEMENT: {
            ...
            if (item && item->type == CONTEXT_TYPE_LIST) {
                for (int i = 0; i < item->data.list.count; i++) {
                    Context *loop_ctx = context_create();
                    // This is a shallow copy, which is not ideal.
                    // A proper implementation would clone the context.
                    loop_ctx->head = ctx->head;
                    context_set_string(loop_ctx, for_node->loop_var, item->data.list.items[i]->data.value);
                    render_node((Node *)for_node->body, loop_ctx, sb);
                    // We don't destroy the loop_ctx because it's sharing the head.
                    // This will leak memory for the new item.
                }
            }
            break;
        }
