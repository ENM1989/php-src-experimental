...
static void lexer_read_char(JulesLexer *l);
static char lexer_peek_char(const JulesLexer *l);
static JulesToken new_token(JulesTokenType type, const char *literal, int line);
static char *read_text(JulesLexer *l);
static char *read_identifier(JulesLexer *l);
static char *read_number(JulesLexer *l);
static char *read_string(JulesLexer *l);
static void skip_whitespace(JulesLexer *l);

JulesLexer *jules_lexer_create(const char *input) {
    JulesLexer *l = (JulesLexer *)malloc(sizeof(JulesLexer));
    if (!l) return NULL;
...
void jules_lexer_destroy(JulesLexer *l) {
    if (l) {
        free(l);
    }
}

JulesToken jules_lexer_next_token(JulesLexer *l) {
    ...
    if (l->state == LEXER_STATE_TEXT) {
        ...
        if (strlen(text) > 0) {
            JulesToken token = new_token(TOKEN_TEXT, text, l->line);
            free(text);
            return token;
        }
        free(text);
    ...
    if (l->state == LEXER_STATE_TAG) {
        if (isalpha(l->ch) || l->ch == '_') {
            ...
            JulesToken token = new_token(TOKEN_IDENTIFIER, ident, l->line);
            free(ident);
            return token;
        }
        if (isdigit(l->ch)) {
            char *num = read_number(l);
            JulesToken token = new_token(TOKEN_NUMBER, num, l->line);
            free(num);
            return token;
        }
        if (l->ch == '"') {
            char *str = read_string(l);
            JulesToken token = new_token(TOKEN_STRING, str, l->line);
            free(str);
            return token;
        }
    }
    ...
    JulesToken token = new_token(TOKEN_ILLEGAL, literal_str, l->line);
    lexer_read_char(l);
    return token;
}
